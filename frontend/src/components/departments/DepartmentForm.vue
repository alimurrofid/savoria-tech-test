...
const handleSubmit = () => {
  errors.name = '';
  if (!form.name.trim()) { errors.name = 'Department name is required.'; return; }
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-5" novalidate>
    <div class="flex flex-col gap-1.5">
      ...
      <InputText v-model="form.name" placeholder="e.g. Human Resources" :class="['w-full', { 'p-invalid': errors.name }]" />
    ...
    <div class="flex justify-end gap-3 pt-2">
      ...
      <Button type="submit" :label="initialData?.id ? 'Save Changes' : 'Create Department'" icon="pi pi-check" :loading="loading" />
    </div>
  </form>
</template>
