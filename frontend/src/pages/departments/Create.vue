...
const handleFormSubmit = async (payload: { name: string }) => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Created', detail: 'Department created successfully.', life: 3000 });
...
<template>
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
      <DepartmentForm :loading="loading" @submit="handleFormSubmit" @cancel="router.push({ name: 'departments.index' })" />
    </div>
  </div>
</template>
