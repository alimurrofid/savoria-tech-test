...
onMounted(async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to load department.', life: 3000 });
...
const handleFormSubmit = async (payload: { name: string }) => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Updated', detail: 'Department updated successfully.', life: 3000 });
    router.push({ name: 'departments.index' });
  } catch (err: any) {
...
<template>
  <div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
      <Button icon="pi pi-arrow-left" text rounded @click="router.push({ name: 'departments.index' })" />
      <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
      ...
      <DepartmentForm v-else :initial-data="department" :loading="saving" @submit="handleFormSubmit" @cancel="router.push({ name: 'departments.index' })" />
    </div>
  </div>
</template>
