  ...
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <DataTable ref="dt" :value="records" dataKey="id" :loading="loading" :paginator="true" :rows="10"
        v-model:filters="filters" :globalFilterFields="['name', 'email']"
        ...
        <Column field="name" header="Name" style="min-width: 180px">
          <template #body="{ data }">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 font-bold text-xs shrink-0">
        ...
        <Column field="email" header="Email" style="min-width: 180px">
          <template #body="{ data }"><span class="text-slate-600">{{ data.email }}</span></template>
        </Column>
        ...
        <Column header="Type" style="width: 100px">
          <template #body="{ data }">
            <Tag :value="data.is_admin ? 'Admin' : 'User'" :severity="data.is_admin ? 'warn' : 'secondary'" />
          </template>
        </Column>
